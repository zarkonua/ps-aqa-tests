...
| API-NOTE-03 | P0 | Positive | Content round-trips | Note created with non-empty content | GET the note | `content` equals what was sent (broken mode blanks it to `""`) | notes-crud.spec |
| API-NOTE-04 | P0 | Positive | List includes created note | ≥1 note created | GET `/api/notes` | `200`, array contains the note; each item has full shape and intact `content` | notes-crud.spec |
| API-NOTE-05 | P0 | Positive | Update note | Note created | PUT `/api/notes/{id}` `{title:"New", content:"New body"}` | `200`; `title`/`content` updated; `id` unchanged; `updated_at` ≥ `created_at` (`created_at` immutability → API-NOTE-10 🐞) | notes-crud.spec |
